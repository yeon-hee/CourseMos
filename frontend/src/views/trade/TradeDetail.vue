<template>
  <div style="height: 100%;">
    <img :src="temp.mainphotourl" style="height: 250px; width: 100%;" />
    <br />
    <div style="text-align: center; margin-top: 8px;">
      <div v-if="feedNo != null">
        <img
          src="../../assets/images/backwards_icon_149034.png"
          width="20px"
          height="20px"
    ...
    <div style="margin: 25px; line-height: 2.0em;">
      ...
      >{{temp.address.region.newaddrfullname}} {{temp.address.newaddr.newaddrfull}}</p>
      <br />
      <img
        ...
        class="detail1"
      />
      <p style="display: inline; margin-left: 6px;">
        영업시간 :
        {{temp.openHour.periodList[0].timeList[0].dayOfWeek}}
        {{temp.openHour.periodList[0].timeList[0].timeSE}}
      </p>
      <br />
    ...
    <div style="margin: 25px; line-height: 1.2em;">
      ...
      <div v-for="menus in menu.bottomList" :key="menus.id " style="line-height: 0.5em;">
        ...
        <div class="hr-sect">
          <div style="float: right; color:rgb(255,81,81);">{{menus.price}}</div>
        </div>
        <br style="clear:both;" />
      </div>
      <br />
      ...
      <p style="display: inline; color:rgb(40,180,0);">{{blog.blogrvwcnt}}</p>
      <br />
      <br />
        ...
        <v-col v-for="(blogs, i) in blog.list" :key="i" cols="12">
          <v-card>
            <a :href="blogs.outlink" style="text-decoration:none;">
              <div
                style="padding:16px 12px 0px 12px; font-size: 15px; color:rgb(20,10,160); font-weight: bold;"
              >{{blogs.title}}</div>
            </a>
            <div class="d-flex flex-no-wrap justify-space-between">
              <v-card-subtitle
                v-text="blogs.contents"
                class="text"
    ...
    <div class="line"></div>
    <div style="margin: 25px; line-height: 1.2em;">
      <h4 style="display: inline;">방문자 리뷰</h4>
      <p style="display: inline; color:rgb(40,180,0);">{{review.allComntcnt}}</p>
      <br />
      <br />
      <div v-for="(reviews, i) in review.list.slice(0, 5)" :key="i">
        <div>
          <p style="float: left; margin-right:13px;">
            <img :src="reviews.profile" style="height: 45px; width: 45px;" />
          </p>
          <div style="font-size: 13px; " class="text2">{{reviews.contents}}</div>
          <p
            style="display: inline; font-size: 11px; color:#8a8a8a;"
          >{{reviews.point}}점 | {{reviews.username}} | {{reviews.date}}</p>
          <br />
